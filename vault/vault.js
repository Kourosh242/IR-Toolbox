...
import { faNum, faBytes, download, hasCrypto, INSECURE_MSG } from '../js/helpers.js';
  ...
  mount(root) {
    ...
    function paneText(p) {
      ...
      const btn = el('button', { class: 'btn primary', onclick: async () => {
        if (!ta.value) { info.textContent = '❌ متنی برای رمزنگاری وارد نشده'; return; } // fix: فایل خالی ساخته نمی‌شود
