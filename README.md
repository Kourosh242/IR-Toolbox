<p align="center">
  <img src="assets/brand/iva-banner.png" alt="IVA — جعبه ابزار آیوا" width="100%">
</p>

<p align="center">
  <strong>جعبه ابزار آیوا</strong> — یک وب‌اپ کاملاً محلی و آفلاین با ۴۵ ابزار کاربردی در ۹ دسته.
</p>

<p align="center">
  <a href="https://kourosh242.github.io/IVA-Toolbox/"><strong>🌐 مشاهده نسخه آنلاین</strong></a>
  ·
  <a href="docs/">📚 ویکی پروژه</a>
  ·
  <a href="LICENSE">📄 مجوز MIT</a>
</p>

---

## ✨ قابلیت‌ها

- 🔒 **کاملاً محلی و آفلاین** — بدون سرور، بدون دیتابیس، بدون آپلود؛ داده‌های شما از دستگاه شما بیرون نمی‌رود.
- 🧰 **۴۵ ابزار در ۹ دسته** — متن، توسعه‌دهنده، طراحی، فایل، محاسبات، زمان، امنیت، سرگرمی و بازی فکری.
- 🛡️ **گاوصندوق IVA** — رمزنگاری متن/فایل با AES-256-GCM و مشتق کلید PBKDF2، با خروجی `.iva256`.
- 📲 **PWA واقعی** — قابل نصب و استفاده‌ی کاملاً آفلاین.
- 🇮🇷 **فارسی و راست‌چین** — با فونت خودمیزبان وزیرمتن (بدون CDN، مناسب ایران).
- 🧩 **رجیستری ماژولار** — افزودن ابزار جدید بدون دست‌کاری هسته اپ.
- 🎨 **پوسته‌های متنوع** — روشن، تاریک، سیستم، نیمه‌شب و شیشه‌ای.
- ⌨️ **پالت فرمان** — جستجوی سریع ابزار با `Ctrl+K` / `Cmd+K`.

## 🗂️ تغییرات نسخه

### v1.3.1
- رفع باگ آفلاین: فهرست کش اولیه Service Worker کامل شد تا همه ماژول‌ها در نصب اولیه و آفلاین هم درست بارگذاری شوند.

### v1.3
- گاوصندوق: کارت‌های فایل (کپی متن / دانلود با نام اصلی) بالای پیش‌نمایش متن قرار گرفتند.
- پاک‌شدن خودکار خروجی با پاک‌شدن ورودی در همه بخش‌ها.

### v1.2
- بخش «هش ← متن» در بررسی هش با دیتابیس محلی.
- گاوصندوق: دکمه «کپی متن» و «دانلود با نام اصلی» برای هر فایل.

### v1.1
- جداکننده هزارگان فارسی، پیکر تقویم شمسی، محاسبه سن با ورودی شمسی.
- بازسازی بررسی هش + بخش «شناسایی و تطبیق هش»، بخش «توضیحات» زیر ابزارها.
- Base64 ← فایل با پیش‌نمایش زنده، تغییر پسوند رمزنگاری به `.iva256`.

### v1.0
- انتشار نخست: ۴۵ ابزار در ۹ دسته + گاوصندوق IVA + PWA آفلاین.

> فهرست کامل و تفصیلی در [docs/changelog.md](docs/changelog.md) آمده است.

## 🚀 اجرای سریع

سایت استاتیک است و **نیازی به build ندارد**:

```bash
cd iva
python3 -m http.server 8000
# سپس: http://localhost:8000
```

جزئیات استقرار و نصب PWA در ویکی: [شروع به کار](docs/start.md).

## 📚 ویکی

- [صفحه اصلی ویکی](docs/index.md)
- [فهرست ابزارها](docs/tools.md)
- [گاوصندوق و امنیت](docs/vault.md)
- [PWA و آفلاین](docs/pwa.md)
- [معماری و کد](docs/architecture.md)
- [توسعه‌دهندگان](docs/dev.md)
- [تغییرات نسخه](docs/changelog.md)

## 📄 مجوز

این پروژه تحت مجوز **MIT** منتشر شده است — به [LICENSE](LICENSE) مراجعه کنید.

---

<p align="center">ساخته‌شده با ❤️ برای استفاده‌ی کاملاً محلی — داده‌های شما از دستگاه شما بیرون نمی‌رود.</p>
