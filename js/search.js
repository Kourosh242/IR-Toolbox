...
function open(onNavigate) {
  ...
  const draw = (q) => {
    ...
    items.forEach((it, i) => {
      ...
      list.append(b);
      if (i === sel && typeof b.scrollIntoView === 'function') b.scrollIntoView({ block: 'nearest' }); // fix: گزینهٔ انتخابی با کلیدهای جهت‌دار از دید خارج می‌شد
    });
  };
  input.addEventListener('input', debounce(() => { render(input.value.trim()); }, 120));
  input.addEventListener('keydown', (e) => {
    if (!items.length && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) { e.preventDefault(); return; } // fix: sel منفی نمی‌شود
