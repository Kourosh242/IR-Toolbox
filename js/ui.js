...
export function textInput(opts = {}) {
  return el('input', { class: `input ${opts.mono ? 'code' : ''}`, type: opts.type || 'text', placeholder: opts.placeholder || '', value: opts.value || '' });
}
...
export function readout(placeholder = 'خروجی این‌جا نمایش داده می‌شود') {
  const box = el('div', { class: 'readout' }, el('span', { class: 'placeholder' }, placeholder));
  const wrap = el('div', {},
