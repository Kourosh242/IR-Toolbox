...
// fix: فاصله/خط‌شکستگی داخل Base64 و padding ناقص (مثل JWT) دیگر خطا نمی‌دهند
const utf8b64decode = (s) => {
  let clean = s.replace(/\s+/g, '');
  if (clean.length % 4 === 1) throw new Error('bad base64');
  clean += '='.repeat((4 - (clean.length % 4)) % 4);
  return new TextDecoder('utf-8', { fatal: true }).decode(Uint8Array.from(atob(clean), (c) => c.charCodeAt(0)));
};
...
register({
  ...
  mount(root) {
    ...
    const val = () => { if (parse() !== undefined) { errBox.style.display = 'none'; out.box.dataset.raw = ''; out.box.textContent = ''; out.box.append(el('span', { class: 'ok-box', style: 'display:inline-block' }, '✅ JSON معتبر است')); } }; // fix: دکمهٔ کپی دیگر خروجی کهنهٔ قبلی را کپی نمی‌کند
...
register({
  ...
  mount(root) {
    ...
    const out = readout();
    const run = () => {
      // fix: حروف عربی (ي ك ة ؤ إ أ)، اعداد فارسی/عربی و تشدید/اعراب هم پوشش داده می‌شوند
