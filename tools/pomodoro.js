...
// fix: AudioContext ساخته‌شده خارج از کلیک کاربر، به‌خاطر سیاست autoplay مرورگر «معلق» می‌ماند و بیپ پایان مرحله پخش نمی‌شد؛
// حالا با کلیک «شروع» ساخته/فعال می‌شود و همان برای بیپ به کار می‌رود.
let actx = null;
const armAudio = () => {
  try {
...
register({
  ...
  mount(root) {
    ...
    const todayKey = () => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`; }; // v1.3.4: تاریخ محلی، نه UTC
    ...
    const setMode = (m) => {
      mode = m;
      const raw = m === 'focus' ? +fIn.value || 25 : m === 'short' ? +sIn.value || 5 : +lIn.value || 15;
      const mins = Math.min(180, Math.max(1, Math.floor(raw))); // fix: مقدار منفی/اعشاری/خیلی بزرگ تایمر را خراب می‌کرد
      total = left = mins * 60;
      modeLbl.textContent = m === 'focus' ? '🎯 تمرکز' : m === 'short' ? '☕ استراحت کوتاه' : '🌴 استراحت بلند';
      modeLbl.className = `badge ${m === 'focus' ? 'ok' : 'info'}`;
      show();
    };
    const stop = () => { clearInterval(tick); tick = null; endAt = null; document.title = 'پومودورو — IR-Toolbox'; }; // fix: عنوان صفحهٔ ابزار (نه خانه) برمی‌گردد
    // fix: تغییر مدت‌ها هنگام توقف، بلافاصله روی نمایشگر اعمال می‌شود
    [fIn, sIn, lIn].forEach((i) => i.addEventListener('input', () => { if (!tick) setMode(mode); }));
    const phaseEnd = () => {
      stop(); beep();
      if (mode === 'focus') {
        cycles += 1;
    ...
    setMode('focus');
    setCleanup(stop); // v1.3.4: با ترک صفحه، تایمر و عنوان مرورگر رها می‌شوند
    root.append(
