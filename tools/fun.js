...
function picker(root, list, label, icon, hint) {
  ...
  const btn = el('button', { class: 'btn primary', onclick: () => { box.textContent = list[Math.floor(Math.random() * list.length)]; } }, `${icon} ${label}`);
  root.append(box, el('div', { class: 'dash-actions' }, btn, copyBtn(() => box.textContent, 'کپی')), counter);
}
