...
register({
  ...
  mount(root) {
    ...
    const reText = () => {
      ...
      }).catch(() => { if (my === textToken) out.set('❌ محاسبهٔ هش ممکن نشد'); }); // fix: خطای Web Crypto بی‌صدا رها نمی‌شود
    };
    ...
    const reFile = async () => {
      ...
      } catch { if (my === fileToken) fOut.set('❌ خواندن یا هش‌کردن فایل ممکن نشد'); } // fix: فایل حذف‌شده/غیرقابل‌خواندن، خطای مدیریت‌نشده می‌داد
    };
...
register({
  ...
  mount(root) {
    ...
    const entropy = () => {
      ...
      if (opts.symbol.checked) pool += 22; // fix: مجموعهٔ نمادها واقعاً ۲۲ نویسه دارد؛ ۳۰ آنتروپی را بیش‌برآورد می‌کرد
