...
register({
  ...
  mount(root) {
    ...
    const run = (hex) => {
      ...
      // fix: ورودی بدون «#» یا سه‌رقمی (مثل 12b98b یا #fff) نرمال می‌شود؛ قبلاً رنگ swatch نامعتبر و خروجی HEX ناقص بود
      const norm = '#' + rgb.map((x) => x.toString(16).padStart(2, '0')).join('');
      swatch.style.background = norm;
      out.set(`HEX: ${norm.toUpperCase()}\nRGB: rgb(${rgb.join(', ')})\nHSL: hsl(${h}, ${s}%, ${l}%)`);
    };
    color.addEventListener('input', () => { hexIn.value = color.value; run(color.value); });
    hexIn.addEventListener('input', () => {
      if (hexToRgb(hexIn.value)) { color.value = '#' + hexToRgb(hexIn.value).map((x) => x.toString(16).padStart(2, '0')).join(''); run(hexIn.value); }
      else if (hexIn.value.trim()) out.set('❌ HEX نامعتبر'); // v1.3.4: بدون خروجی کهنه
    });
    run('#12b98b');
...
register({
  ...
  mount(root) {
    ...
    const out = readout();
    const angField = field(`زاویه: ${faNum(140)} درجه`, ang); // v1.3.3: برچسب زنده با عدد زاویه
    const run = () => {
      const css = `linear-gradient(${ang.value}deg, ${c1.value}, ${c2.value})`;
      prev.style.background = css;
      out.set(`background: ${css};`);
      angField.querySelector('.lbl').textContent = `زاویه: ${faNum(ang.value)} درجه`;
    };
    [c1, c2, ang].forEach((x) => x.addEventListener('input', run)); run();
    root.append(el('div', { class: 'grid2' }, field('رنگ اول', c1), field('رنگ دوم', c2)), angField, prev, out.root);
  }
});
