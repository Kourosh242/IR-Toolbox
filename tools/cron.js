...
// fix: نام‌های استاندارد cron انگلیسی‌اند (MON, JAN)؛ قبلاً با فهرست فارسی مقایسه می‌شدند و هرگز تطبیق نمی‌خوردند
const EN_WD = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
const EN_MO = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'];

/* یک فیلد را به مجموعه مجاز + توضیح فارسی تبدیل می‌کند */
function parseField(expr, min, max, names) {
  if (typeof expr !== 'string' || !expr.trim()) throw new Error('عبارت کامل نیست — ۵ بخش لازم است');
  const enNames = names === WD ? EN_WD : names === MO ? EN_MO : null;
  const val = (t) => {
    if (enNames && /^[A-Za-z]+$/.test(t)) {
      const i = enNames.indexOf(t.toLowerCase().slice(0, 3));
      if (i >= 0) return min + i;
    }
    const n = Number(t);
  ...
  for (const part of expr.split(',')) {
    ...
    if (m[1] === '*') { lo = min; hi = max; }
      ...
      lo = a; hi = b ?? a; // fix: نام تکی (مثل MON) hi را undefined می‌گذاشت
    } else { lo = hi = val(m[1]); if (m[2]) hi = max; }
    const step = m[2] ? Number(m[2]) : 1;
    if (step < 1) throw new Error('گام باید ≥۱ باشد');
    if (lo > hi) throw new Error(`بازه «${m[1]}» معکوس است`);
    for (let v = lo; v <= hi; v += step) set.add(v === 7 && names === WD ? 0 : v);
    if (m[1] === '*' && step > 1) parts.push(`هر ${faNum(step)} تا`);
    else if (lo === hi) parts.push(names ? names[(lo - min) % names.length] ?? faNum(lo) : faNum(lo));
    else parts.push(`${names?.[(lo - min) % names.length] ?? faNum(lo)} تا ${names?.[(hi - min) % names.length] ?? faNum(hi)}`);
  }
  // fix: پرچم «هر» (star) صریح ذخیره می‌شود؛ قبلاً با اندازهٔ set تشخیص داده می‌شد که برای روز هفته (۷→۰) هرگز درست نبود
  return { set, desc: parts.join('، '), star: expr.trim() === '*' };
}

function describe(c) {
  try {
    const segs = c.trim().split(/\s+/);
    if (segs.length !== 5 || segs.some((x) => !x)) throw new Error('عبارت کامل نیست — دقیقاً ۵ بخش لازم است'); // fix: عبارت ۲ تا ۴ بخشی کرش می‌کرد
    const [mi, h, d, mo, wd] = segs;
    const F = [
...
function matches(F, d) {
  ...
  const domStar = dom.star, wdStar = wd.star; // fix: «0 0 1 * *» قبلاً هر روز اجرا می‌شد چون wdStar هرگز true نمی‌شد
