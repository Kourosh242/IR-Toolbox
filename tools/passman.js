...
import { faNum, uid, download, textBlob } from '../js/helpers.js';
...
function screenMain(box, rerender) {
  ...
  form.append(
    ...
    el('div', { class: 'dash-actions' },
      el('button', { class: 'btn primary sm', onclick: async () => {
        ...
        await saveBlob(); form.style.display = 'none'; editing = null; toast('💾 ذخیره و رمزنگاری شد', 'ok'); draw(); drawStats();
      } }, '💾 ذخیره'),
  ...
  function entryCard(e) {
    ...
    const title = el('div', { style: 'display:flex;gap:8px;align-items:center;flex-wrap:wrap' },
      el('b', {}, e.title),
      e.url ? el('a', { href: e.url, target: '_blank', rel: 'noopener', style: 'font-size:.75rem' }, '🌐 باز کردن') : null,
  ...
  box.append(
    ...
    field('جست‌وجو', q), form, chBox, list,
