 ...
 * ورودی‌ها با AES-256-GCM + PBKDF2(600k) رمزنگاری و در localStorage ذخیره می‌شوند.
 * شمارش تکرار در فیلد it ذخیره می‌شود؛ داده‌های قدیمی بدون it با ۲۵۰k باز می‌شوند.
...
/* fix: بعد از «پاک‌سازی همهٔ داده‌ها» در تنظیمات، حافظهٔ مدیر رمز هم قفل می‌شود */
...
const strength = (pw) => {
  ...
  if (/[^a-zA-Z0-9]/.test(pw)) pool += 22; // fix: هم‌خوان با مجموعهٔ ۲۲ نمادی تولیدکننده (۳۰ آنتروپی را بیش‌برآورد می‌کرد)
  ...
  form.append(
    ...
    el('div', { class: 'dash-actions' },
      el('button', { class: 'btn primary sm', onclick: async () => {
        ...
        // fix: بازگردانی هنگام خطای ذخیره — قبلاً در حالت «ویرایش»، ورودیِ موجود به‌کلی حذف می‌شد
        const snapshot = entries.map((x) => ({ ...x }));
        ...
        try { await saveBlob(); } catch { entries = snapshot; toast('❌ ذخیرهٔ محلی ممکن نشد — حافظهٔ مرورگر پر یا در دسترس نیست', 'err'); draw(); drawStats(); return; } // v1.3.4
  ...
  function entryCard(e) {
    ...
    const title = el('div', { style: 'display:flex;gap:8px;align-items:center;flex-wrap:wrap' },
      ...
      el('button', { class: 'btn danger sm', onclick: async () => {
        ...
        // fix: خطای ذخیره بی‌صدا رها می‌شد و فهرست حافظه با localStorage ناهماهنگ می‌ماند
        try { await saveBlob(); } catch { entries = prev; toast('❌ حذف ذخیره نشد — حافظهٔ مرورگر در دسترس نیست', 'err'); return; }
        drawStats(); draw();
      } }, '🗑'));
