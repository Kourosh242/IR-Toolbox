...
register({
  ...
  mount(root) {
    ...
    // v1.3.4: فقط در صورت نبودن xmlns تزریق کن — xmlns تکراری XML را نامعتبر و دانلود را خراب می‌کرد
    const withXmlns = (s) => (/xmlns=/.test(s) ? s : s.replace('<svg ', '<svg xmlns="http://www.w3.org/2000/svg" '));
    const dlSvg = () => { if (lastSvg) download('qr.svg', textBlob(withXmlns(lastSvg), 'image/svg+xml')); };
    const dlPng = () => {
      if (!lastSvg) return;
      const svg = withXmlns(lastSvg);
      const img = new Image();
      img.onload = () => {
